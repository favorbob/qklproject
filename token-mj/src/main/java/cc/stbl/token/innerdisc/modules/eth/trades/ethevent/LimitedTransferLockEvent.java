...
@Component
public class LimitedTransferLockEvent extends AbstractVrTokenEvent {

    @Autowired
    private LimitedTransferLockEvent _this;
    ...
    protected void listenerEventStartBy(Long start, VRToken vrToken) {
        vrToken.limitedTransferLockEventObservable(new DefaultBlockParameterNumber(start), DefaultBlockParameterName.LATEST)
                .subscribe(lockedTransferEventResponse -> _this.processEvent(lockedTransferEventResponse));
    }

    private void processEvent(VRToken.LimitedTransferLockEventResponse event) {
        String tradeId = event.tradeNo;
        logger.info("LimitedTransferLockEvent={}`transactionHash={}",event.tradeNo,event.log.getTransactionHash());
        ...
        BigDecimal lockBalanceOfTo= UnitConvertUtils.toEther(event.lockBalanceOfTo);
        recordFlow(up,limitedBalanceOfFrom,lockBalanceOfTo);
        tradeRecordEventListener.sendTradeRecordEvent(up);
        setStartValue(up.getAtBlockNumber() - 1);
    }

    private void recordFlow(EthTradeRecord up, BigDecimal limitedBalanceOfFrom, BigDecimal lockBalanceOfTo) {
