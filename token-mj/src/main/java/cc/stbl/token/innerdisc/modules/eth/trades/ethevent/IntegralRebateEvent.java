...
@Component
public class IntegralRebateEvent extends AbstractVrTokenEvent {
    ...
    @Autowired
    private EthIntegralRebateListener rebateListener;

    @Override
    protected void listenerEventStartBy(Long start, VRToken vrToken) {
        vrToken.integralRebateEventObservable(new DefaultBlockParameterNumber(start), DefaultBlockParameterName.LATEST)
                .subscribe(event -> {
             String lockId = "distributedLock:event:IntegralRebateEvent:" + event.tradeNo;
    ...
    protected void processEvent(VRToken.IntegralRebateEventResponse event){
        logger.info("listener_IntegralRebateEventResponse_{}", JSON.toJSONString(event));
        EthReturnedIntegral ri = returnedIntegralService.get(event.tradeNo);
        if(ri.getStatus() == TradeConsts.TRADE_STATUS_SUCCESS) return;
        ri.setBalance(UnitConvertUtils.toEther(event.balance));
        ri.setLimitedBalance(UnitConvertUtils.toEther(event.limitedBalance));
        ri.setIntegral(UnitConvertUtils.toEther(event.integral));
        ...
        integralFlowService.createFlow(ri.getTransactionHash(),ri.getAtBlockNumber(), BEnum.INTEGRAL_REBATE,
                ri.getId(),ri.getSuccessDate(),UnitConvertUtils.toEther(vrTokenManager.integralOf(ri.getAddress()))
        ...
        assetFlowService.createFlow(
                ...
                UnitConvertUtils.toEther(vrTokenManager.balanceOf(ri.getAddress())),
        ...
        assetFlowService.createFlow(
                ...
                UnitConvertUtils.toEther(vrTokenManager.limitBalanceOf(ri.getAddress())),
