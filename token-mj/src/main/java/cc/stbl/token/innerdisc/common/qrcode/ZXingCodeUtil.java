package cc.stbl.token.innerdisc.common.qrcode;
...
public class ZXingCodeUtil {
	...
	public BufferedImage getQR_CODEBufferedImage(ZXingConfig zxingconfig) throws Exception {
		...
		try {
			...
			for (int x = 0; x < w; x++) {
				for (int y = 0; y < h; y++) {
					image.setRGB(x, y, bm.get(x, y) ? zxingconfig.getQRCode() : zxingconfig.getQRCodeBgd());
				}
			}
	...
	/**
	 * 这个工具类 ZXingCodeUtil 是提供二维码图片生成的工具 首先使用的时候需要实例化 ZXingCodeUtil 然后实例化参数
	 * ZXingConfig 和 LogoConfig 通过下面的演示可以详细看参数是按照什么循序进行设置 最后调用 ZXingCodeUtil 中方法
	...
	public static void main(String[] args) throws WriterException {
		...
		try {
			// 生成二维码
			File file = new File("/Users/ylh-007/eclipse-workspace/QRCode/src/com/stbl/images/Michael_QRCode.png");
			...
			zxingconfig.setLogoPath("/Users/ylh-007/eclipse-workspace/QRCode/src/com/stbl/images/tooopen_sy_220999936848.jpg"); // 设置Logo图片
